...
Les modalités possibles de la variable *chaussures* sont **no** ou **yes**. Voici le modèle considéré : 

$$malade = \beta_{0} + \beta_{chaussureyes} * chaussureyes $$



```{r}
### MODELE DE REGRESSION ###
res <- glm(malade ~ chaussures, family="binomial", data=data)
res
confint(res, level=0.95)

```
Nous obtenons $\beta_{1}=-0.2156$ et $IC_{95%} = [-0.5565847, 0.1268667]$.
Le groupe de référence est représenté par la modélité "no". Ainsi la valeur obtenue pour $\beta_{chaussureyes} - \beta_{chaussureno} = ln(\frac{odds(malade=1|chaussures ="yes")}{odds(malade=1|chaussures="no")})$ est -0.2156, soit :

$$\frac{odds(malade=1|chaussures="yes")}{odds(malade=1|chaussures="no")}= \exp(0.2156) = 0.806$$

Ainsi, les individus portant des chaussures ont 
